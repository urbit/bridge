...
class Download extends React.Component {
  handleSave = (event) => {
    ...
    const urbit_registration = Object.entries(wallets).reduce((acc, [k, v]) => {
  ...
  render() {
    ...
    const nextButtonClass = downloaded ? 'btn btn-primary' : 'btn shape-gray-10'

    return(
      <div className={'col-md-6'}>
        <h2 className={'mb-4'}>Your wallets have been generated. Download the files to a storage medium you trust.</h2>
        ...
        <div
          ...
          style={{borderBottom:'2px solid #222' }}
          >
          <div className={'col-md-6 text-mono text-600'}>{'urbit-wallets.zip'}</div>
          <div className={'col-md-6'}><p className={'mv-0'}>Paper wallet PNG files for <code>{`${props['profile.shipCount']}`}</code> ships, organized by recommended custody tier. This contains your private keys, keep it safe. </p></div>
        </div>
        ...
        <div
          ...
          style={{borderBottom:'2px solid #222' }}
          >
          <div className={'col-md-6 text-600 text-mono'}>{'urbit-registration.txt'}</div>
          <div className={'col-md-6 mv-0'}><p className={'mv-0'}>{`A file containing the public addresses of your wallets. You need to upload this to Registration next.`}</p></div>
        </div>
...

        {
          didClickDownload
            ? <div>
                <p>Please verify that the above files have been successfully downloaded. You may wish to unpack the zip file and inspect the contents. If you open your .png wallets on your screen, ensure you are in a secure location.</p>
                <CheckBox
                ...
                <Button
                  ...
                  text={'Next →'}
                  disabled={ !downloaded }
                  onClick= { () => setGlobalState({ route: '/Done' , 'currentStep': NEXT_STEP_NUM })}
                />
              </div>
            : <div />
        }
