...
const infoManagementSeed = ship => {
  ...
  return <Tile colorClass={colorClass} key={`info-${ship}-manage`} col1={`Management Proxy`} col2={''} />
...
const infoVotingSeed = ship => {
  ...
  return <Tile colorClass={colorClass} key={`info-${ship}-voting`} col1={`Voting Proxy`} col2={''} />
}
...
const infoSpawnSeed = ship => {
  ...
  return <Tile colorClass={colorClass} key={`info-${ship}-spawn`} col1={`Spawn Proxy`} col2={''} />
}
...
const infoMasterTicket = ship => {
  ...
  return <Tile colorClass={colorClass} key={`info-${ship}-master`} col1={`Master Ticket`} col2={''} />;
};
...
const infoMasterTicketShard = ship => {
  ...
  const colorClass = getColor(classOf, 'master');
  return [
    <Tile colorClass={colorClass} key={`info-${ship}-master-1`} col1={`Master Ticket Shard 1 of 3`} col2={''} />,
    <Tile colorClass={colorClass} key={`info-${ship}-master-2`} col1={`Master Ticket Shard 2 of 3`} col2={''} />,
    <Tile colorClass={colorClass} key={`info-${ship}-master-3`} col1={`Master Ticket Shard 3 of 3`} col2={''} />,
  ];
};
...
class Generate extends React.Component {
  ...
  handleGenerate = async () => {
    ...
    const reshapedWallet = wallets.reduce((acc, {ship, wallet}) => ({...acc, [ship]: wallet }), {})
  ...
  render() {
    ...
    const ALL_SUCCESS =
      es === ENY_SUCCESS &&
      ws === DERIVATION_SUCCESS &&
      ps === PAPER_SUCCESS;

    const NONE_START =
      es === ENY_NOSTART &&
      ws === DERIVATION_NOSTART &&
      ps === PAPER_NOSTART;

    const ANY_ERROR =
      es === ENY_FAILURE ||
      ws === DERIVATION_FAILURE ||
      ps === PAPER_FAILURE;

    const ANY_PENDING =
      es === ENY_PENDING ||
      ws === DERIVATION_PENDING ||
      ps === PAPER_PENDING;
    ...
    return(
      ...
      <div className="col-md-3 col-md-offset-1 mt-9">
        <div className="fixed" style={{width: '230px'}}>
          ...
          <p className="text-sm" style={{lineHeight: '16px'}}>
            Extremely valuable. Cold storage on a 3 part sharded paper wallet. Store each shard in a separate geographic location with protection against fire, water and other elements.
          </p>
          <div className="col-md-12 bg-high h-2 mt-4"></div>
          <h4 className="text-600 mt-2">High Friction</h4>
          <p className="text-sm" style={{lineHeight: '16px'}}>
            Very valuable. Cold storage on a paper wallet in a different location than your computer, such as a safe.
          </p>
          <div className="col-md-12 bg-medium h-2 mt-4"></div>
          <h4 className="text-600 mt-2">Medium Friction</h4>
          <p className="text-sm" style={{lineHeight: '16px'}}>
            Store cold in a paper wallet; can be stored in a desk drawer in a trusted environment.
          </p>
      ...
      <div className={'col-md-9 btn-tray col-'}>
        ...
        <p className={'col-md-8'}>{"If you have a large number of ships, this may take several minutes and the page may hang or become unresponsive. If you close this browser window, you will have to start from the beginning."}</p>
        ...
        <div className={'flex h-12 mt-8 mb-4'}>
          ...
          <Button
            ...
            onClick= { () => setGlobalState({ route: '/Download' , 'currentStep': NEXT_STEP_NUM })} />
        </div>
      </div>
